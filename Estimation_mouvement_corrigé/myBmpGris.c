...
/** Methode de creation d'une image Bmp "standard" (pas de LUT, pas d'alpha, codage sur 24 bits)
  ...
  */
BmpImg createBmpImg( char* str, int dimX, int dimY ) { //Fonction permettant de déclarer une structure avec nos propre
                                                       //dimensions dans l'optique de créer une image BitMap
...
/** Methode de creation d'un header Bmp "standard" (14 octets, pas de LUT, pas d'alpha, codage sur 24 bits)
  ...
  */
void createBmpHeader( BmpImg* bmpImg ) { //Déjà appelée
...
/** Methode de creation d'un header DIB "standard" (40 octets, pas de LUT, pas d'alpha, codage sur 24 bits)
  * @param bmpImg une struct BmpImg ou les champs dimX, dimY et de taille (xxSize) sont deja remplis
  */
void createDibHeader( BmpImg* bmpImg ) { //Déjà appelée
...
/** Methode de lecture d'une image bitmap
  ...
  * @return l'image
  */
BmpImg readBmpImage( char* imgName ) { //Fonction permettant de créer une structure BmpImg AVEC les valeur d'un fichier
                                       //Bitmap qu'on lit
                                       //PREUMS
	BmpImg bmpImg = initImg( );
	...
	//MODIFICATION ESSENTIEL AU BON DEROULEMENT DU PROGRAMME
	if (fImg == NULL) {
...
/** Methode auxiliaire de lecture d'une image bitmap
  ...
  * 		dimX et dimY
  */
void readHeader( FILE* fImg, BmpImg* bmpImg ) { //Déjà appelée par readBmpImage
	// Bmp Header
...
/** Methode auxiliaire de lecture d'une image bitmap
  ...
  */
void readImg( FILE* fImg, BmpImg* bmpImg ) { //Permet de lire une image BitMap et d'écrire les valeurs dans l'image de la structure
                                             //(en chaines de charactère)
                                             //Extrait le premier plan couleur à chaque tour ?
...
/** Methode d'ecriture d'une image bitmap
  ...
  */
void writeBmpImage( char* imgName /*image Bitmap*/, BmpImg* bmpImg ) { //Permet d'écrire dans un fichier bitmap (pour modifier l'image)
                                                                       //(à partir de l'image dans la structure)
	int i, j;
...
/** Methode allouant l'espace memoire necessaire aux 3 plans couleur d'une image Bmp
  ...
  */
void createImg( BmpImg* bmpImg ) { //Déjà appelée dans la première (alloue l'espace nécessaire pour créer la matrice générale)
